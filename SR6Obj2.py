...
class Obj(object):
    ...
    def transformar(self, vertice):
        ...
        vertice_t = imagen.matrixMult(self.ViewPort, imagen.matrixMult(self.Perspeciva, imagen.matrixMult(self.View, imagen.matrixMult(self.Modelo, vertice4D))))
    ...
    def load(self, traslacion = (0, 0, 0), escala = (1, 1, 1), rotacion = (0, 0, 0), textura = None):
        ...
        for cara in self.caras:
            ...
            if not textura:
                ...
                imagen.triangle(v1, v2, v3, n1, n2, n3)
            else:
...
# ------- Funciones para mostrar cada angulo -------

def MediumShot():
    imagen.glInit() # Inicializar SR1
    imagen.glCreateWindow(800,800) # Cambia el tamano del window a 800x800
    imagen.glClearColor(0, 0, 0) # Cambia el color de la ventana a negro
    imagen.glClear() # Clear a ventana
    imagen.glViewPort(0, 0, 800, 800) # Especificar en donde se dibujara
    obj = Obj('Porygon22.obj', 'Porygon22.mtl') # Cargar el contenido del obj
    # Donde se encuentra la camara.
    obj.VistaCam(V3(0, 1, 2.5), V3(0, 1, 0), V3(0, 1, 0))
    # Dibujar el contenido
    obj.load((0, 0, 0), (1, 1, 1), (0, 0, 0))
...
def HighAngle():
    ...
    # Dibujar el contenido
    obj.load((0, 0, 0), (0.7, 0.7, 0.7), (1, 1, 0))
    imagen.glFinish() # Generar la imagen
...
def LowAngle():
    ...
    obj.VistaCam(V3(0, 0, 2.5), V3(0, 0, 0), V3(0, 1, 0))
    # Dibujar el contenido
    obj.load((0.1, -1, 0), (1.7, 1.7, 1.7), (0, 1, -0.3))
    imagen.glFinish() # Generar la imagen
...
def DutchAngle():
    ...
    imagen.glClear() # Clear a ventana
    imagen.glViewPort(0, 0, 800, 800) # Especificar en donde se dibujara
    obj = Obj('sphere.obj', '') # Cargar el contenido del obj
    # Donde se encuentra la camara. En este caso esta girada para crear el efecto Dutch Angle (se rota la camara y no el modelo)
    obj.VistaCam(V3(0, 0, 2.5), V3(0, 0, 0), V3(-1, 2.5, 0))
    # Dibujar el contenido del obj sin escala, sin traslacion ni rotacion
    obj.load((0, 0, 0), (1, 1, 1), (0, 0, 0))
    imagen.glFinish() # Generar la imagen
